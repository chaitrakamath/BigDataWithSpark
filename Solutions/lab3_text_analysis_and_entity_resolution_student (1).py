...
def simpleTokenize(string):
    ...
    """
    alphaNumeric = re.sub('[^a-zA-Z0-9_ ]', ' ', string)
    lowerCase = alphaNumeric.strip().lower()
...
def tokenize(string):
    ...
    """
    words = [w for w in simpleTokenize(string) if not w in stopwords]
    return words
...
def countTokens(vendorRDD):
    ...
    """
    counts =  vendorRDD.map(lambda (a, b): len(b))
    return counts.sum()
...
def tf(tokens):
    ...
    tfValues = {}
    for value in tokens:
        if value not in tokenDict.keys():
            tokenDict[value] = 1
        else:
            tokenDict[value] += 1
    for keys in tokenDict:
        tfValues[keys] = float(tokenDict[keys]) / len(tokens)
    return tfValues
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.flatMap(lambda token: (token)).map(lambda token: (token, 1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b)
    return (tokenSumPairTuple.map(lambda token: (token[0], float(N) / token[1])))
...
similaritiesBroadcast = (crossSmall
                         ...
                         .cache())
print 'SimilaritiesBroadcast:', similaritiesBroadcast.take()
...
avgSimDups = (trueDupsRDD.map(lambda (a, b): b[0]).sum()) / trueDupsCount
# print avgSimDups
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda data: (data[0], tokenize(data[1])))
googleFullRecToToken = google.map(lambda data: (data[0], tokenize(data[1])))
...
def fastCosineSimilarity(record):
    ...
    s = 0
    for item in tokens:
        s += amazonWeightsBroadcast.value[amazonRec][item] * googleWeightsBroadcast.value[googleRec][item] 
