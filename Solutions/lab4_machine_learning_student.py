...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avgRatings = sum(IDandRatingsTuple[1]) / float(len(IDandRatingsTuple[1]))
    countsAndAvgTup = (IDandRatingsTuple[0], (numOfRatings, avgRatings))
    return countsAndAvgTup
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda ratings : (ratings[1], ratings[2]))
                          .groupByKey())
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda (a, b) : math.pow((b[1] - b[0]), 2)))
...
validationForPredictRDD = validationRDD.map(lambda a : (a[0], a[1]))
...
myModel = ALS.train(trainingRDD, rank = bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda a : (a[0], a[1]))
